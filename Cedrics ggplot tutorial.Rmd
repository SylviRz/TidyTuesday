...
* You can also download the R script containing only the code [here](https://cedricscherer.netlify.app/files/ggplot-tutorial-cedric-raw.R).
...
p1 <- ggplot(chic, aes(x = factor(year), y = temp)) +
  ...
  labs(x = "Year", y = "Temperature (°F)") +
  scale_y_continuous(limits = c(0, 50)) +
  ggtitle("scale_y_continuous(limits)")
...
You can also set a non-default font for all text elements of your plots, for more details see section ["Working with Themes"](#themes). I am going to use *Roboto Condensed* as new default font for all the plots that follow.
...
<details>
  <summary>&#128129; <i>You can achieve the same by setting the legend name to `NULL`, either via `scale_color_discrete(name = NULL)` or `labs(color = NULL)`. Expand to see example.</i></summary>
```{r legend-title-null}
ggplot(chic, aes(x = date, y = temp, color = season)) +
  geom_point() +
  labs(x = "Year", y = "Temperature (°F)") +
