...
hike_data_short3 <- hike_data_short2[sample(nrow(hike_data_short2)),]
...
plot4<-ggplot(hike_data_short3, aes(x=as.numeric(as.factor(features)), y=n)) +
  geom_area( fill="bisque3") +
  geom_line(color="bisque4", size=2) +
  geom_point(size=3, color="bisque4") +
...
theme(text = element_text(family = "Andale Mono"), legend.position = "none", # change all text font and move the legend to the bottom
  ...
  labs(title = "On a mountain high",
       subtitle = "Mountain Views are the most common feature on Washington Trails. \\ Followed by: hiking with your dog on a leash, and Wildflowers.",
       x = "Most frequent features of hikes", y=NULL,
       caption = "DataViz by @SylviRz for #TidyTuesday, data from Washington Trails Association")
plot4
ggsave("mountainhigh1.png")

# to do:
#   - subtitle line break
# - order the hike features a bit differently
# - include features as axis labels for the numeric var...
